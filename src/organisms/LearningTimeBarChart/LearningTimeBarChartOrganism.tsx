...
class LearningTimeBarChartOrganism extends React.Component<IProps, any> {
  componentDidMount() {
    const svg = d3.select(`#${this.props.id || 'svgWrapper'}`)
      .append('svg')
  ...
  const barGroupBack = barGroup.append('rect')
    ...
    .attr('x', (d, i) => (((svgWidth / numberOfBars) * i) + barPadding) - (numberOfBars / 2))
  ...
  renderYAxisText = (d: IBarData) => {
    if (this.props.type === 'SMALL') return `${Math.floor(d.duration / 60)} menit`;
